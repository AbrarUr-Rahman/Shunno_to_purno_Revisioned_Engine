...
@onready var grid_container = $GridContainer
@onready var next_button = $Button
@onready var popup_label = $PopupLabel  # Optional: for showing text messages (e.g., PopupPanel or AcceptDialog)
...
func _ready():
	...
	for i in range(card_points.size()):
		...
		var card = grid_container.get_child(i)
		card.modulate = Color(1, 1, 1, 1)
		card.scale = Vector2(1, 1)  # Reset scale
	...
	for i in GameState.selected_card_indices:
		...
		var card = grid_container.get_child(i)
		card.modulate = Color(0.5, 0.5, 0.5)
		card.scale = Vector2(1.05, 1.05)  # Apply selection animation
...
func _on_card_clicked(card_index: int) -> void:
	...
	if selected_cards[card_index]:
		...
		if selected_count < max_limit:
			...
			print("You have reached the selection limit!")
			show_popup_message("You have reached the limit!")  # Optional popup
...
func _on_next_page_pressed() -> void:
	...
	if selected_count == required_count:
		...
		print("You must select exactly %d cards to proceed!" % required_count)
		show_popup_message("Select exactly %d cards to continue!" % required_count)
...
# ✅ Optional: show temporary popup message (if PopupPanel or AcceptDialog is used)
func show_popup_message(msg: String) -> void:
	if popup_label:
		popup_label.dialog_text = msg
		popup_label.popup_centered()
